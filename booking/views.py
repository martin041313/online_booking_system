    ...
    if 'application/json' in request.META.get('CONTENT_TYPE'):
        try:
            ...
            if(data['post']=='yes'):
                ...
                if userInfo.balance < hr.price:
                    string = "<title>302 Found</title>balance not enough.\n"
                else:
                    hr.stat = '1'
                    hr.save()
                    #userInfo.balance -= hr.price
                    
                    userInfo.pay(hr.price, "This is a hotel payment")
                    userInfo.save()
                    string = "<title>302 Found</title>Resource has been modified.\n"
            else:
                string = "<title>302 Found</title>Resource has not been modified.\n"
        ...
        if(request.POST['post']=='yes'):
            if userInfo.balance < hr.price:
                    messages.add_message(request, messages.INFO, 'balance not enough.\n')
            else:
                    hr.stat = '1'
                    hr.save()
                    #userInfo.balance -= hr.price
                    ...
                    messages.add_message(request, messages.SUCCESS, 'pay successfully enough.\n')
        else:
            messages.add_message(request, messages.INFO, "not payed.\n")
        return HttpResponseRedirect('/booking/my_record/hotel/') 
        ...
        b = userInfo.balance
        return render_to_response('payment_hotel.html', {'rr':hr, 'b':b}, context_instance=RequestContext(request))
...
def pay_flight(request, pk_id):
    ...
    if(fr.stat=='1'):
    ...
    elif 'application/json' in request.META.get('CONTENT_TYPE'):
        try:
            data = json.loads(request.body)               
            if(data['post']=='yes'):
                if userInfo.balance < fr.price:
                    string = "<title>302 Found</title>balance not enough.\n"
                else:
                    fr.stat = '1'
                    fr.save()
                    #userInfo.balance -= fr.price
                    
                    userInfo.pay(fr.price, "This is a flight payment")
                    userInfo.save()
                    string = "<title>302 Found</title>Resource has been modified.\n"
            else:
                string = "<title>302 Found</title>Resource has not been modified.\n"
        ...
        b = userInfo.balance
        return render_to_response('payment_flight.html', {'rr':fr, 'b':b}, context_instance=RequestContext(request))
...
def my_flight_record(request):
    ...
    '''
    frs = flight_record.objects.filter(user_id=request.user).order_by('-id')
...
def my_hotel_record(request):
    ...
    you can see all your booking items
    '''
    hrs = hotel_record.objects.filter(user_id=request.user).order_by('-id')
...
def flight_record_view(request, pk_id):
    ...
    try:
        ...
        if 'application/json' in request.META.get('CONTENT_TYPE'):
            ...
            return render_to_response('fr_detail.html', {'rr':fr, }, \
                                      context_instance=RequestContext(request))
...
def hotel_record_view(request, pk_id):
    ...
    try:
        ...
        if 'application/json' in request.META.get('CONTENT_TYPE'):
            ...
            return render_to_response('hr_detail.html', {'rr':hr, }, \
                                          context_instance=RequestContext(request))
...
def book_hotel(request, pk_id):
    ...
    if start!=None:
        ...
        start_day = datetime.date(2013, 6, 1)#datetime.date.today()
    ...
    if request.method == 'GET':
        sr = hotel_record.objects.filter(hotel_id=h).filter(~Q(scores = -1))
...
def book_flight(request, pk_id):
    ...
    if request.method =='GET':
        sr = flight_record.objects.filter(flight_id=f.id).filter(~Q(scores = -1))  
        paginator = Paginator(sr, 10)
        page = request.GET.get('p', '1')
        try:
            gg = paginator.page(page)
    ...
    fr = flight_record.objects.create(user_id=request.user, \
                ...
                book_time=datetime.datetime.now().time(),price = price,stat='0')
    fr.save()
    f.save()
    return HttpResponseRedirect('/booking/my_record/flight/')
...
def search_flight(request):
    ...
    if 'application/json' in request.META.get('CONTENT_TYPE'):
        ...
        if form.is_valid():
            ...
            return render_to_response('search_flight.html', { "fls":g2,'form':form}, context_instance=RequestContext(request))
    else:
        form = flightForm()
        return render_to_response('search_flight.html', { "fls":g2,'form':form}, context_instance=RequestContext(request))
    try:
    ...
    if 'application/json' in request.META.get('CONTENT_TYPE'):
        ...
        for r in gg:
            ...
            data['company_id'] = company.objects.get(pk = data.get('company_id')).name
            for x in data:
                data[x] = str(data[x])
            #assert False
            dlist.append(data)
        ...
        return render_to_response('search_flight.html', { "fls":g2,'form':form,'rs':gg}, context_instance=RequestContext(request))
#@login_required
def search_hotel(request):
    ...
    if 'application/json' in request.META.get('CONTENT_TYPE'):
            try:
                ...
                string += str(e)
                string += "<title>400 Bad Request</title>Badly-formed JSON sent.\n"
                response = HttpResponse(string,status=401,)
                response['Vary'] = 'Accept-Encoding'
                response['Content-Length'] = len(string)
                return response    
        ...
        if form.is_valid():
            ...
            return render_to_response('search_hotel.html', {"hls": g1,'form':form }, context_instance=RequestContext(request))
    else:
        form = hotelForm()
        return render_to_response('search_hotel.html', {"hls": g1,'form':form }, context_instance=RequestContext(request)) 
    ...
    if 'application/json' in request.META.get('CONTENT_TYPE'):     
        ...
        for r in gg:
            ...
            data['room_type'] = 'big' if data['room_type']=='2' else "standard"
            for x in data:
                data[x] = str(data[x])
            #assert False
            dlist.append(data)
        ...
        return render_to_response('search_hotel.html', {"hls": g1, 'form':form,'rs':gg, }, context_instance=RequestContext(request))   
...
def get_flight(request,pk_id):
    f = flight.objects.get(pk=pk_id)
    data = model_to_dict(f)
    data['leave_date'] = data.get('leave_date').strftime('%Y-%m-%d')
    data['leave_time'] = data.get('leave_time').strftime('%H:%M:%S')
    data['arrive_date'] = data.get('arrive_date').strftime('%Y-%m-%d')
    data['arrive_time'] = data.get('arrive_time').strftime('%H:%M:%S')
    ...
    for x in data:
        data[x] = str(data[x])
    encodeData = json.dumps(data,sort_keys=True,indent=2)
    return HttpResponse(encodeData, content_type="application/json")
def get_hotel(request,pk_id):
    f = hotel.objects.get(pk=pk_id)
    data = model_to_dict(f)
    data['city_id'] = city.objects.get(pk=data.get('city_id')).name
