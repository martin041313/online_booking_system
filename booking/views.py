...
def book_hotel(request, pk_id):
    ...
    return render_to_response('ensure_hotel.html', {'rs':rs, 'h':h, 'sr':sr}, context_instance=RequestContext(request))
...
def book_flight(request, pk_id):
    ...
    if request.method == 'POST':
        if request.POST.get('post') == 'yes':
            ...
            fr = flight_record.objects.create(user_id=request.user, flight_id=f, num=num, book_date=datetime.date.today(), book_time=datetime.datetime.now().time())
            fr.save()
            f.save()
            return HttpResponseRedirect('/booking/my_record/flight/')
    elif request.method == 'PUT' and 'application/json' in request.META.get('CONTENT_TYPE'):
    ...
    return render_to_response('ensure_flight.html', {'f':f, 'sr':sr}, context_instance=RequestContext(request))
# 这是搜索航班的页面 
@login_required
