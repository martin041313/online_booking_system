...
class AccountInfo(models.Model):
    ...
    isVip = models.BooleanField(blank=False,default=False)
    isLogin = models.BooleanField(blank=False,default=False)

    def __unicode__(self):
        return '%s' % self.user.username
    def pay(self, amount,brief):
        self.balance = self.balance-amount
        self.save()
        # add payment record to payDetail
        payDetail.objects.create(user=self,amount=amount,balance=self.balance,brief=brief)
    def setVip(self,vip):
        self.isVip=vip
...
class payDetail(models.Model):
    user = models.ForeignKey(AccountInfo)
