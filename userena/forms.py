...
from userena.models import AccountInfo
...
attrs_dict2 = {'class': 'required,input-block-level'}
...
class SignupForm(forms.Form):
    ...
    #verify code field
    valification=CaptchaField(label="* Valification")
    ...
    def save(self):
        ...
        AccountInfo.objects.create(user=new_user)
        #AccountInfo.objects.create(new_user)
        return new_user
...
def identification_field_factory(label, error_required):
    ...
    return forms.CharField(label=label,
                           widget=forms.TextInput(attrs=attrs_dict2),
                           max_length=75,
...
#class ToPayForm(forms.Form):
#    #email = forms.EmailField(widget=forms.TextInput(attrs=dict(attrs_dict,
#     #                                                          maxlength=75)),
##                             label=_(u"New email"))
#    payTo = forms.CharField()
#    amount = forms.DecimalField(min_value=decial.Decimal(0))
#
#
#    def __init__(self, user, *args, **kwargs):
#        """
...
#
#        """
#        super(ToPayForm, self).__init__(*args, **kwargs)
#        if not isinstance(user, get_user_model()):
#            raise TypeError, "user must be an instance of %s" % get_user_model().__name__
#        else: self.user = user
#
#    def save(self):
#        return self.user.userena_toPay.change_email(self.cleaned_data['email'])
#
