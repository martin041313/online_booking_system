...
from userena.models import payDetail
    ...
    if user:
        if userena_settings.USERENA_USE_MESSAGES:
            messages.success(request, _('Your email address has been changed.'),
                             fail_silently=True)
            #UserInfo.objects.create(user = user.user)
...
def profile_detail(request, username,
    ...
    **Context**
    ...
    if not extra_context: extra_context = dict()
    account = AccountInfo.objects.get(user=user)
    extra_context['profile'] = user.get_profile()
    extra_context['hide_email'] = userena_settings.USERENA_HIDE_EMAIL
    extra_context['userAccount'] = account
    extra_context['payDetail'] =account.paydetail_set.order_by('time')[:5].reverse()
    return ExtraContextTemplateView.as_view(template_name=template_name,
                                            extra_context=extra_context)(request)
